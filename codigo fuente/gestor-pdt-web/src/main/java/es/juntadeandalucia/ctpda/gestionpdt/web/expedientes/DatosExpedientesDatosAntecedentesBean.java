...
@ViewScoped
@Slf4j
public class DatosExpedientesDatosAntecedentesBean extends BaseBean implements Serializable {
	...
	@Getter @Setter
	private Boolean permisoEditarAntecedentes;
	...
	private void cargarPermisosUsuario() {
		...
		permisoEditarAntecedentes = listaCodigoPermisos.contains(Constantes.PERMISO_EDIT_ANTECEDENTES);
	}
	...
	private void inicializaLazyModel() {
		...
		lazyModel.setPreproceso((a,b,c,filters)->{				
			...
			filters.put("#antecedentesNotNull", new MyFilterMeta(null));
		});
	...
	public void onChangeIncluir(DetalleExpdteTram detalleRow) {
		...
		try {
			...
			detalle.setAntecedentesExpediente(detalleRow.getAntecedentesExpediente());
		} catch (Exception e) {
			facesMsgError(e.getMessage());
			log.error(e.getMessage());
			e.printStackTrace();
	...
	public void guardar() {
		try {
			...
			final String msg = getMessages("antecedentes", "guardados.correctamente");
