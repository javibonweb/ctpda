...
@ViewScoped
@Slf4j
public class FirmasAbiertasBean extends BaseBean implements Serializable {
	/**
	 * serial id
	 */
	...
	private static final String FORMFIRMASABIERTAS = "formListadoFirmasAbiertas";
	public static final String VOLVERLISTADOFIRMASABIERTAS = "_volverFirmasAbiertas_";
	...
	public void init() {
		...
		selectedTipoTramiteIdFiltro = tipoTramiteService.findTipoTramiteActivoFirm().getId();
		...
		navegacionBean.setTxtMigaPan(Constantes.LISTADO_FIRMAS_ABIERTAS);
	}
	...
	public void buscar()
	{
		...
		PrimeFaces.current().ajax().update(FORMFIRMASABIERTAS);
	}
	...
	private void filtrosLazyModel (Map<String, FilterMeta> filters) {
		...
		if (firmanteFiltro != null && !firmanteFiltro.isEmpty()) {
			filters.put("firmante", new MyFilterMeta(firmanteFiltro));
		}
	...
	public String redireccionMenu() {
		return ListadoNavegaciones.LISTADO_FIRMAS_ABIERTAS.getRegla();
	}
	...
	public String onEditar(Long idTram) {
		...
		final ContextoVolver contexto = volverBean.crearContexto(ListadoNavegaciones.LISTADO_FIRMAS_ABIERTAS.getRegla(), Constantes.VOLVERFIRMASABIERTAS);
		cargarFiltrosEnContexto(contexto);
		
		JsfUtils.setSessionAttribute(Constantes.VUELTA_FIRMAS_ABIERTAS, true);
	...
	{
		contexto.put("selectedTipoTramiteIdFiltro", tipoTramiteService.findTipoTramiteActivoFirm().getId());
	...
	private void cargarFiltros() {
		final ContextoVolver cv = volverBean.recogerContexto(Constantes.VOLVERFIRMASABIERTAS);
		...
		if(cv != null) {
			...
			this.firmanteFiltro = (String) cv.get("firmanteFiltro");
