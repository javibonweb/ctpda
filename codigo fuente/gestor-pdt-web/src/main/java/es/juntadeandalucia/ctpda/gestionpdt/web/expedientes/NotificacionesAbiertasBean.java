...
@ViewScoped
@Slf4j
public class NotificacionesAbiertasBean extends BaseBean implements Serializable {
	/**
	 * serial id
	 */
	...
	private static final String FORMNOTIFICACIONESABIERTAS = "formListadoNotificacionesAbiertas";
	public static final String VOLVERLISTADONOTIFICACIONESABIERTAS = "_volverNotificacionesAbiertas_";
	...
	@Getter
	@Setter
	private TramitesAbiertosMaestra selectedNotificacionesAbiertas;
	...
	public void init() {
		...
		listaValoresDominioCanalSalida = valoresDominioService.findValoresDominioByCodigoDominio(Constantes.COD_DOM_CAN_SAL);
		...
		navegacionBean.setTxtMigaPan(Constantes.LISTADO_NOTIFICACIONES_ABIERTAS);
	}
	...
	public void buscar()
	{
		...
		PrimeFaces.current().ajax().update(FORMNOTIFICACIONESABIERTAS);
	}
	...
	private void filtrosLazyModel (Map<String, FilterMeta> filters) {
		...
		if (selectedCanalSalidaIdFiltro != null) {
			...
			filters.put("canalSalida", new MyFilterMeta(descripcion));
		}
	...
	public String redireccionMenu() {
		return ListadoNavegaciones.LISTADO_NOTIFICACIONES_ABIERTAS.getRegla();
	}
	...
	public String onEditar(Long idTram) {
		...
		final ContextoVolver contexto = volverBean.crearContexto(ListadoNavegaciones.LISTADO_NOTIFICACIONES_ABIERTAS.getRegla(), Constantes.VOLVERNOTIFICACIONESABIERTAS);
		cargarFiltrosEnContexto(contexto);
		
		JsfUtils.setSessionAttribute(Constantes.VUELTA_NOTIFICACIONES_ABIERTAS, true);
	...
	private void cargarFiltros() {
		final ContextoVolver cv = volverBean.recogerContexto(Constantes.VOLVERNOTIFICACIONESABIERTAS);
