...
public class TareasExpedienteService extends AbstractCRUDService<TareasExpediente> {
	 ...
	 * hacen falta
	 */
	public TareasExpedienteService(@Autowired MathsQueryService mathsQueryService,
			@Autowired TareasExpedienteRepository tareasExpedienteRepository) {
		super(mathsQueryService, tareasExpedienteRepository, QTareasExpediente.tareasExpediente);
		...
		this.joinBuilder = query -> {
			...
			final QResponsablesTramitacion qResponsablesTramitacion = new QResponsablesTramitacion(
					"qResponsablesTramitacion");
	...
	public void checkSiPuedoGrabar(TareasExpediente dto) throws BaseException {
		...
		if (dto.getId() == null && null != dto.getDocumentoExpedienteTramite()
	...
	protected BooleanBuilder aniadirFiltrosCustom(List<FiltroDTO> filtros) {
		...
		for (FiltroDTO f : filtros) {
			if (f.getCampo().equals(FILTRO_LISTA_IDS_RESP)) {
				...
				bb = bb.and(QTareasExpediente.tareasExpediente.responsableTramitacion.id.in(idsResp));
			}
		}
	...
	public List<TareasExpediente> findTareasPendientesExpediente(Long idExp) {
		return this.tareasExpedienteRepository.findByExpedienteIdAndActivaAndSituacion(idExp, Boolean.TRUE,
				TareasExpediente.SITUACION_PENDIENTE);
	}

	public List<TareasExpediente> findTareasPendientesTramite(Long idTramite) {
		return this.tareasExpedienteRepository.findByTramiteExpedienteIdAndActivaAndSituacion(idTramite, Boolean.TRUE,
				TareasExpediente.SITUACION_PENDIENTE);
	}

	public List<TareasExpediente> findTareasPendientesExpedienteDeResponsable(Long idExp, Long idResp) {
		return this.tareasExpedienteRepository.findByExpedienteIdAndResponsableTramitacionIdAndActivaAndSituacion(idExp,
				idResp, Boolean.TRUE, TareasExpediente.SITUACION_PENDIENTE);
	}

	public List<TareasExpediente> findTareasPendientesTramiteDeResponsable(Long idTramite, Long idResp) {
		return this.tareasExpedienteRepository
				.findByTramiteExpedienteIdAndResponsableTramitacionIdAndActivaAndSituacion(idTramite, idResp,
						Boolean.TRUE, TareasExpediente.SITUACION_PENDIENTE);
	}
	...
	public List<TareasExpediente> findTareasPendientesTramiteDeResponsablesDeUsuario(Long idTramite, Long idUsuario) {
		...
		return lista.stream().map(UsuariosResponsables::getResponsable)
	...
	public ValoresDominio findTipoTarea(String codigo) {
		return valoresDominioService.findValoresDominioByCodigoDomCodValDom("TAREAS", codigo);
	}
	...
	private List<CfgTareas> findCfgTareas(TareasExpediente tarea) throws BaseException {
		...
		final Long[] idsTipoTramite = tramiteExpedienteService
				.getIdsTipoTramiteSubtramite(tarea.getTramiteExpediente());
		...
		return this.cfgTareasService.findTiposTareasActivasCfg(exp.getValorTipoExpediente().getId(), idsTipoTramite[0],
				idsTipoTramite[1], null != idDocExpTramite);
	}
	...
	public CfgTareas getCfgTareaActual(TareasExpediente tarea) throws BaseException {
		...
		if (listaCfg.isEmpty()) {
			throw new BaseException("La lista de CfgTareas para el tipo de la tarea " + tarea.getId()
					+ " no devolvió ningún elemento.");
		}

		if (listaCfg.size() > 1) {
			throw new BaseException("La lista de CfgTareas para el tipo de la tarea " + tarea.getId()
					+ " devolvió más de un elemento.");
		}
	...
	public void aplicarFechaLimite(TareasExpediente tarea, CfgTareas cfgTarea) {
		...
		// Si no hemos calculado la fecha límite según plazo hago el cálculo según la
		// fecha de inicio como siempre
	...
	private PlazosExpdte findPlazoAbiertoTarea(TareasExpediente tarea) {
		...
		return plazosExpdteService.findPlazosExpdteActivosNoCumplidosByExpTipPla(tarea.getExpediente().getId(),
				tipo.getId());
	}
	...
	public void aplicarDatosTramite(TareasExpediente tarea, Long idTramiteExpediente, Long idDocExpTramite) {
		if (null != idDocExpTramite) {
			final DocumentosExpedientesTramites docTramExp = documentosExpedientesTramitesService
					.obtener(idDocExpTramite);
	...
	public void cerrarTarea(TareasExpediente tarea, Usuario usuarioCierre, String motivo) throws BaseException {
		...
		// El cierre se hace desde el diálogo, puede que se hayan cambiado datos, lo que
		// implica a otras operaciones
	...
	public void cerrarTareaInmediatamente(TareasExpediente tarea, Usuario usuarioCierre, String motivo)
	...
	// Cierra todas las tareas de expediente (sin trámite) cuya configuración
	// especifique L_FINAL_ALTA_TRAM = 1
	private void cerrarTareasPorAlta(Expedientes exp, Usuario usuario, String motivo, Predicate<CfgTareas> condicion)
			throws BaseException {
		List<TareasExpediente> tareasExp = this.findTareasPendientesExpediente(exp.getId());

		for (TareasExpediente tarea : tareasExp) {
			// De las tareas abiertas finalizo aquellas cuya configuración asociada así lo
			// indique
	...
	public void rechazarTarea(TareasExpediente tarea, Usuario usuarioRechaza, String motivo) throws BaseException {
		...
		ResponsablesTramitacion respUsuCreador = responsablesTramitacionService
				.findResponsablePorDefectoUsuario(idUsuCreador);
		tarea.setResponsableTramitacion(respUsuCreador);

		// Adjuntamos motivo a las observaciones
		String cabeceraMotivo = "MOTIVO DE RECHAZO (" + usuarioRechaza.getNombre() + " - "
				+ FechaUtils.stringFechaYHora() + "):\n";
	...
	public TareasExpediente guardar(TareasExpediente tarea, Usuario usuario) throws BaseException {
		...
		if (esAlta) {
			...
			if (tarea.getCambiaResponsable()
					&& Boolean.TRUE.equals(
							cfgTarea.getInformeCambioResponsable() && null != cfgTarea.getValorTipoTareaSiCambiaResp())
	...
	private void crearTareaCambioResponsable(TareasExpediente tarea, CfgTareas cfgTarea, Usuario usuario)
			throws BaseException {
		...
		tarea2.setResponsableTramitacion(
				responsablesTramitacionService.findResponsablePorDefectoUsuario(tarea.getUsuarioCreador().getId()));
	...
	private void cambiarUsuarioCreador(TareasExpediente tarea, Usuario usuario) {
		// actualmente este es el único sitio donde se aplica el responsable (el combo
		// del diálogo no usa ajax)
		// quiere decir que podemos obtener el responsable anterior y comparar

		if (null != tarea.getId() && tarea.getCambiaResponsable() && this.existeTareaSiguiente(tarea)) {
	...
	public class AccionTarea {
		...
		public void aplicarDatosTarea(TareasExpediente tarea) {
			/**
			 * Informamos los datos de expediente, trámite, etc. A partir de estos
			 * obtendremos los datos de tipo de expediente, tipo de trámite, etc. con los
			 * que buscar en la tabla de CfgTareas.
			 */
		...
		protected void accionInvalida() throws BaseException {
			throw new BaseException("Se produjo un error en la aplicación. Consulte con su administrador.",
	...
	private class AccionTareaExpediente extends AccionTarea {
		...
		public void setIdDocExpTram(Long idDocExpediente) {
			throw new UnsupportedOperationException(
					"La accion de expediente no puede informar el id de documento-exp.");
		}
	...
	public AccionTarea nuevaAccionTareaTramite(String tipoAccion, Usuario usuarioCreador, Long idExpediente,
			Long idTramite) {
		return new AccionTareaTramite(tipoAccion, usuarioCreador, idExpediente, idTramite);
	}

	public AccionTarea nuevaAccionTareaDocumento(String tipoAccion, Usuario usuarioCreador, Long idTramite,
			Long idDocExpTram) {
		return new AccionTareaDocumento(tipoAccion, usuarioCreador, idTramite, idDocExpTram);
	}

	public AccionTarea nuevaAccionTareaTarea(String tipoAccion, Usuario usuarioCreador, Long idTareaOrigen)
			throws BaseException {
	...
	// ---------
	// Recuperar las entidades correspondientes a los IDs almacenados en la propia
	// acción
	...
	private void aplicarExpediente(AccionTarea accion) throws BaseException {
		...
		throw new BaseException(
				"El expediente de la acción no puede ser nulo. Usar accion.setExpediente o accion.setIdExpediente.");
	}
	...
	private void aplicarTipoTarea(AccionTarea accion) {
		...
		if (null != accion.getCodTipoTarea()) {
			accion.setValorTipoTarea(
					valoresDominioService.findValoresDominioByCodigoDomCodValDom("TAREAS", accion.getCodTipoTarea()));
		}
	}
	...
	public void crearTareasAuto(AccionTarea accion) throws BaseException {
		...
		// Cierro antes para que las tareas no se cierren a sí mismas despues de
		// guardarse
	...
	private List<CfgTareas> findCfgTareasAccion(AccionTarea accion) {
		return (accion.tipoAccion.equals(AccionTarea.ACCION_DIRECTA)) ? findCfgTareasDirectas(accion)
	...
	private List<CfgTareas> findCfgTareasDirectas(AccionTarea accion) {
		...
		final List<CfgTareas> cfgTareas = cfgTareasService.findCfgTareas(idTipoExp, idsTipoTr[0], idsTipoTr[1],
				idTipoTarea, doc);
	...
	private List<CfgTareas> findCfgTareasSiguientes(AccionTarea accion) {
		...
		final List<CfgTareas> cfgTareas = cfgTareasService.findCfgTareasSiguientes(idTipoExp, idsTipoTr[0],
				idsTipoTr[1], idTipoTareaOrigen, doc);
		return filtrarPorTipoLlamada(cfgTareas, accion);
	}
	...
	private List<CfgTareas> filtrarPorTipoLlamada(List<CfgTareas> cfgTareas, AccionTarea accion) {
		...
		case AccionTarea.ACCION_ACEPTAR:
			filtro = CfgTareas::getAutoAceptar;
			break;
		case AccionTarea.ACCION_ALTA:
			filtro = CfgTareas::getAutoAlta;
			break;
		case AccionTarea.ACCION_FIN:
			filtro = CfgTareas::getAutoFinalizar;
			break;
		// AccionTarea.ACCION_DIRECTA
		default:
			filtro = cfg -> Boolean.TRUE;
		}
	...
	private ResponsablesTramitacion seleccionarResponsable(AccionTarea accion, CfgTareas cfgTarea) {
		...
		// Si la selección básica no devuelve nada, probar con la tarea origen de la
		// acción
		if (resp == null && (Boolean.TRUE.equals(cfgTarea.getEsResponsableAnterior()))
				&& (null != accion.getTareaOrigen())) {
	...
	private void anyadirObservaciones(TareasExpediente tarea, String txt) throws BaseException {
		...
		if (!StringUtils.isEmpty(txt)) {
			...
			StringBuilder sb = new StringBuilder(StringUtils.isBlank(obs) ? StringUtils.EMPTY : obs);

			if (sb.length() > 0) {
				sb.append("\n\n");
			}

			obsExp = observacionesExpedientesService.guardarObservacionesExpedientes(obsExp, sb.append(txt).toString(),
					COD_VAL_DOM_TIPOBS_TAR, tarea.getExpediente());

		} else {
			obsExp = observacionesExpedientesService.guardarObservacionesExpedientes(obsExp, "", COD_VAL_DOM_TIPOBS_TAR,
					tarea.getExpediente());
		}
	...
	// ***************************
	/*
	 * Se realiza la siguiente comprobación para la tarea: - si existe alguna tarea
	 * posterior (que tenga el ID de tarea mayor que el id actual), - que esté
	 * asignada a un responsable distinto del de la tarea que se está analizando -
	 * que esté en situación de activa y pendiente - que esté asociada al mismo
	 * trámite o a algún subtrámite del trámite de la tarea (para esto último,
	 * buscar con el id trámite en el campo id tramite superior de los trámites del
	 * expediente).
	 */
	...
	public List<TareasExpediente> findDependencias(Long idTarea) {
		...
		BooleanExpression expr = criterioTareaDependiente(tarea, tram);
		Iterable<TareasExpediente> it = this.crudRepository.findAll(expr,
				QTareasExpediente.tareasExpediente.fechaLimite.asc());
	...
	private BooleanExpression criterioTareaDependiente(TareasExpediente tarea, TramiteExpediente tram) {
		...
		final BooleanExpression exprDistResp = qTarea.responsableTramitacion.id.ne(idRespTarea)
				.and(qTarea.bloqueoDistintoResponsable.eq(Boolean.TRUE));
		final BooleanExpression exprMismoResp = qTarea.responsableTramitacion.id.eq(idRespTarea)
				.and(qTarea.bloqueoMismoResponsable.eq(Boolean.TRUE));
	...
	public void crearTareaEDPRT(Long idTram) throws BaseException {
		...
		boolean usrEsDelRespTram = usuariosResponsablesService.esResponsableDeUsuario(respTram.getId(),
				usuCreacion.getId());
		...
		if (!usrEsDelRespTram) {
			...
			boolean usrEsDelRespExp = usuariosResponsablesService.esResponsableDeUsuario(respExp.getId(),
					usuCreacion.getId());
			if (!usrEsDelRespExp) {
	...
	public List<TareasExpediente> findTareasExpActivasRevRevt(Long id, String tipoTareaRev, String tipoTareaRevt) {
